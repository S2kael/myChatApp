...
class Chat extends Component {
    ...
    messageOnKeyPressHandler = (event) =>{
        if (event.shiftKey) {
    ...
    renderMessages = (messages) => {
        ...
        messages.map((message, i) => {
            if (message.author !== ''){
                listMessages.push(<Message key = {i} timestamp = {message.created_at} currentUser = {this.props.user.userid} author = {message.author} contents = {message.content}/>)
            }
        });
        return listMessages;
    }
                            ...
                            <div className="bottom">
                                <div className="form">
                                    <textarea className="form-control" id="chat-message-input" placeholder="Type message..."
                                        rows={1} defaultValue={""} value = {this.state.message} onChange = {this.messageChangeHandler} onKeyUp={this.messageOnKeyUpHandler} onKeyPress={this.messageOnKeyPressHandler} />
                                    <button type="button" className="btn prepend" onClick={this.sendMessageHandler} id="chat-message-submit">
